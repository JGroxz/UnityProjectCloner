...
namespace UnityProjectCloner
{
	...
	public class ProjectClonerWindow : EditorWindow
	{
        ...
        /// </summary>
        public bool IsClone => ProjectCloner.IsClone();

        /// <summary>
        /// Returns true if project clone exists or currently open project is a clone itself.
        /// </summary>
        public bool IsCloneCreated => ProjectCloner.GetCloneProjectPath() != string.Empty;
                ...
                if (IsCloneCreated)
                {
                    ...
                    if (GUILayout.Button("Delete clone"))
                    {
                        bool delete = EditorUtility.DisplayDialog(
                            "Delete the clone?",
                            $"Are you sure you want to delete the clone project '{ProjectCloner.GetCurrentProject().name}{ProjectCloner.CloneNameSuffix}'? If so, you can always create a new clone from ProjectCloner window.", 
                            "Delete", 
                            "Cancel");
                        if (delete)
                        {
